...
/**
 * Testosteronium — User Console schema.
 *
 * Two tables back the end-user helpdesk: `tickets` (a request scoped to the
 * device that opened it) and `ticket_comments` (the conversation thread between
 * the end user and the support agent).
 *
 * `device_id` mirrors the identity reported by the silent background check-in,
 * so a ticket is permanently bound to the device/user that created it.
 */
